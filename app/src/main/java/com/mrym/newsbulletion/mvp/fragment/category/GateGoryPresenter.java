...
//        mvpView.loadingSuccess(newsList);
        addSubscription(apiStores.getNewsList("headline","T1348647909107",0), new SubscriberCallBack<>(new ApiCallback<Map<String, List<NewsSummary>>>() {
            @Override
...
//                }
                try {
                    List<NewsSummary> wgat= model.get("T1348647909107");
//                    List<NewsSummary> wgat2 = new Gson().fromJson(model.get("T1348647909107").toString(), new TypeToken<List<NewsSummary>>() {
//                    }.getType());
                    Log.i("NewsSummary",wgat.get(0).getEname());
//                    List<NewsSummary> wgat3=JsonUtils.fromJsonList(model.get("T1348647909107").toString(),NewsSummary.class);
                    mvpView.loadComplete();
